...
export default function AdminSolicitudesSuministrosPage() {
  ...
  async function loadClientsOnce() {
    // Si tu /api/admin/clientes ya devuelve { user_id, label }, perfecto.
    // Si devuelve otro shape, te lo ajusto.
    const r = await fetch("/api/admin/clientes?page=1&pageSize=500", { cache: "no-store" });
  ...
  async function updateStatus(id, nextStatus) {
    ...
    try {
      ...
      // ✅ Si estás viendo pendientes, lo removemos inmediatamente
      if (String(status).toLowerCase() === "pendiente") {
        setRows((prev) => prev.filter((x) => x.id !== id));
        if (Number.isFinite(Number(count))) setCount((c) => Math.max(0, Number(c || 0) - 1));
      }
    } catch (e) {
      setError(String(e?.message || e));
    } finally {
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-semibold text-black">Solicitudes de suministros</h1>
          </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
          ...
          <div>
            <div className="text-xs font-semibold text-gray-700 mb-1">Tamaño de página</div>
            <select
              value={String(limit)}
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-gray-700">
                        ...
                        <div className="text-[11px] text-gray-500 truncate">ID: {r.id}</div>
                      </td>

                      <td className="px-4 py-3">
                        <div className="font-semibold text-black truncate max-w-[240px]">
                          {r.client_name || "—"}
                        </div>
                        {r.client_email ? (
                      ...
                      <td className="px-4 py-3">
                        <div className="font-semibold text-black truncate max-w-[320px]">
                          {r.suministro_nombre || "—"}
                        </div>
