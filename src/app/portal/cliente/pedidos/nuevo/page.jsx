...
export default function NuevoPedidoPage() {
  ...
  const addOne = (p) => {
    ...
    setCart((prev) => {
      ...
      return {
        ...prev,
        [p.id]: { id: p.id, nombre: p.nombre, price: p.price, qty },
      };
    });
  };
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-sm text-white/60">
            Precios aplicados: <span className="text-white/80 font-medium">{priceTier}</span>
          </p>
        </div>
              ...
              <div key={p.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-white/50">Stock: {p.stock ?? "-"}</div>
                  </div>

                  <button
                    onClick={() => addOne(p)}
                    className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-2 text-xs text-white/90 hover:bg-emerald-400/15 transition"
                  >
                    Agregar
                  </button>
                </div>
                ...
                <div key={it.id} className="rounded-2xl border border-white/10 bg-black/20 p-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => addOne({ id: it.id, nombre: it.nombre, price: it.price })}
                        className="rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-white/80 hover:bg-white/10 transition"
                      >
                        +
                      </button>
